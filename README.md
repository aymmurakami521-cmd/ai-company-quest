...
- **`restricted` では「理由」も金額になりうる**ため、比率の理由のうち
  `computed`（AI関連コストが0でない）と `undefined_zero_denominator`（ちょうど0）は
  1つの `withheld_by_disclosure` に畳みます。片方だけ伏せると消去法で0が読めるためです。
  代わりに**「権限により非表示」という理由は画面とpayloadに明示**します
  （`docs/value-rate-design.md` §11.5）。
...
`restricted` では比率と、**理由のうち金額そのものを述べるもの**（AI関連コストが0／0でない）を伏せ、
**「権限により非表示」**と表示します（0倍とは別の表示で、値としての0は返しません）。
金額未確定・期間不一致などの**構造的な理由はそのまま出します**（運用者が直すべき箇所だからです）。

**ARK利用料は分母に入りません**（§8.1 の定義どおり `business_value / ai_cost`）。
ARK利用料まで含めた All-in ROI / TCO は**別指標として後から足せる構造**だけを用意しており、
既存の `benefit_cost_ratio` の名前を再利用する指標は registry が拒否します。
