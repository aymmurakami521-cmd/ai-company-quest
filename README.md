...
部屋の並び順と、**固定席**（組織定義のroster社員の席）の位置は組織定義だけで決まり、
誰が出社しても、ウィンドウの大きさを変えても動きません。
一方で **`未所属` に並ぶ roster外actorの枠は、その actor がいる間だけ現れます**
（組織定義に無い相手なので、固定席を持ちません）。社長室も組織定義ではなく
`state.player` 由来なので、playerが居ないときは現れません。
...
- **描き切れない席**: 上限を超えた分は黙って落とさず、canvas下部に固定文言＋整数だけで
  ...
  **件数だけを出して問題を隠さない**ためで、部屋が溢れただけで席が溢れていない場合にも
  この行は出ます。DOMの社員一覧・凡例・logは従来通り**全actor**を
